...
module["exports"] = function (n) {
    ...
    t["sendToFundebug"] = function (e, t, n, r) {
        if (e["metaData"] = a["copyWithoutCircle"](e["metaData"]), function (e) {
            ...
            log.error('page_router==>', e.Page);
            let arr = [];
            e.breadcrumbs.forEach(item => {
                if (item.method !== "log"
                ) {
                    ...
                    if (item.method !== 'warn') {
                        ...
                        if (item.args) {
                            ...
                            log.error('baba-->',Array.prototype.slice.apply(item.args)[0])
                            arr.push(Array.prototype.slice.apply(item.args)[0])
                        } 
                    }else {                       
                        log.info('info-->', item);;
                    }    
            ...
            log.error('networkType==>', e.networkType);
            log.error('time==>', e.time);
            log.error('error-type==>', e.type);
            let data = {
                page: e.Page,
                breadcrumbs: arr,
                error: e.error,
            ...
            // http.post( `https://api.wisharedu.com/v1/default/wx-log`,
            // { message: data },
