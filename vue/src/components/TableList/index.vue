	...
	<fullscreen ref="fullscreen" @change="fullscreenChange">
		...
		<div  v-if="tableShow" class="table-content">
			<el-table 
				...
				style="width: 100%">
				<el-table-column v-if="selectionColumn && layoutShow('selectionColumn')" type="selection" min-width="55" align="center"></el-table-column>
				...
				<el-table-column
					...
					<template slot-scope="scope">
						...
						<div  v-if="column.custom" :class=" showOverflowTooltip ? 'beyond-ellipsis' : ''"><slot :name="column.value" :data="scope"></slot></div>
						<div v-else :class=" showOverflowTooltip ? 'beyond-ellipsis' : ''">
							<span v-for="(space,index) in scope.row._level" v-if="index === 0 && !column.hideExpandIcon" :key="space" :class="index == 0 ? '' : 'ms-tree-space'"/>
							<span v-if="iconShow(index,scope.row,column.hideExpandIcon)" class="tree-ctrl" @click="toggleExpanded(scope.$index)">
